function [idxbest, Cbest, sumDbest, Dbest] = kmeans_norand(X, k, varargin)
...
%                 C(1,:) = X(1,:);
%                 index(1) = 1;
                [~, min_i] = min(distfun_norand(X,mean(X),distance));
                C(1,:) = X(min_i,:);
                index(1) = min_i;

                % Select the rest of the seeds by a probabilistic model
                for ii = 2:k                    
                    sampleProbability = min(distfun_norand(X,C(1:ii-1,:),distance),[],2);
                    ...
                    [~, max_i] = max(sampleProbability);
                    fixed_sample = zeros(size(sampleProbability));
                    fixed_sample(max_i) = 1;
                    
                    [C(ii,:), index(ii)] = datasample(S,X,1,1,'Replace',false,...
                        'Weights',fixed_sample);
                end
        end
        ...
        D = distfun_norand(X, C, distance, 0, rep, reps);
        [d, idx] = min(D, [], 2);
        ...
        try % catch empty cluster errors and move on to next rep
            ...
            nonempties = find(m>0);
            D(:,nonempties) = distfun_norand(X, C(nonempties,:), distance, iter, rep, reps);
            d = D((idx-1)*n + (1:n)');
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                [C(changed,:), m(changed)] = gcentroids_norand(X, idx, changed, distance);
                D(:,changed) = distfun_norand(X, C(changed,:), distance, iter, rep, reps);
                    ...
                    switch emptyact
                        ...
                        case 'singleton'
                            for i = empties
                                ...
                                m(i) = 1;
                                idx(lonely) = i;
                                D(:,i) = distfun_norand(X, C(i,:), distance, iter, rep, reps);
                                
                                % Update clusters from which points are taken
                                [C(from,:), m(from)] = gcentroids_norand(X, idx, from, distance);
                                D(:,from) = distfun_norand(X, C(from,:), distance, iter, rep, reps);
...
function D = distfun_norand(X, C, dist, iter,rep, reps)
...
function [centroids, counts] = gcentroids_norand(X, index, clusts, dist)
